...
export class PartyInfo {
  ...
  static fromCBORValue(value: unknown): PartyInfo {
    const v = value as [Uint8Array | null, Uint8Array | null, Uint8Array | null]
...
export class SuppPubInfo {
  ...
  static fromCBORValue(value: unknown): SuppPubInfo {
    const v = value as [number, Uint8Array] | [number, Uint8Array, Uint8Array]
...
export class KDFContext {
  ...
  static fromBytes(data: Uint8Array): KDFContext {
    ...
    const v = decodeCBOR(data) as
      ...
      | [number, unknown, unknown, unknown, Uint8Array]

    assertEqual(Array.isArray(v), true, 'not an array')

    if (v.length === 4) {
      return new KDFContext(
        v[0],
        PartyInfo.fromCBORValue(v[1]),
        PartyInfo.fromCBORValue(v[2]),
        SuppPubInfo.fromCBORValue(v[3])
      )
    } else if (v.length === 5) {
      return new KDFContext(
        v[0],
  ...
  constructor(
    ...
    partyVInfo: PartyInfo,
    suppPubInfo: SuppPubInfo,
    suppPrivInfo: Uint8Array | null = null
  ) {
